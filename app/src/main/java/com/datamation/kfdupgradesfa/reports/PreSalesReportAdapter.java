...
public class PreSalesReportAdapter extends BaseAdapter {
    ...
    public View getView(int i, View view, ViewGroup viewGroup) {
        ...
        int pieces = Integer.parseInt(OrderModel.getFORDDET_QTY());
        int cases = Integer.parseInt("0");
        int units = Integer.parseInt("0");
        int reachOfItem = Integer.parseInt(OrderModel.getFORDDET_SEQNO());

        cases = cases + (pieces/units);
        pieces = pieces % units;

        itemname.setText(OrderModel.getFORDDET_ITEMNAME());
        caseqty.setText(""+cases);
        pieceqty.setText(""+pieces);
        amt.setText(OrderModel.getFORDDET_AMT());
        reach.setText(""+OrderModel.getFORDDET_SEQNO());
