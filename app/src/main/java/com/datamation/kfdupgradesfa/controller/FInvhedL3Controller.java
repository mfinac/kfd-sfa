...
import com.datamation.kfdupgradesfa.model.Last3Invoice;
...
public class FInvhedL3Controller {
    ...
    public void createOrUpdateFinvHedL3(ArrayList<FInvhedL3> list) {
        ...
        try {
            ...
            String sql = "INSERT OR REPLACE INTO " + ValueHolder.TABLE_FINVHEDL3 + " (DebCode,RefNo,RefNo1,TotalAmt,TotalTax,TxnDate,TxnDateTime) " + " VALUES (?,?,?,?,?,?,?)";
            ...
            for (FInvhedL3 finvHedL3 : list) {
                ...
                stmt.bindString(7, finvHedL3.getFINVHEDL3_TXN_DATETIME());
    ...
    public int deleteAll() {
        ...
        Cursor cursor = null;
        try {
            cursor = dB.rawQuery("SELECT * FROM " + ValueHolder.TABLE_FINVHEDL3, null);
            count = cursor.getCount();
            if (count > 0) {
                int success = dB.delete(ValueHolder.TABLE_FINVHEDL3, null, null);
    ...
    public ArrayList<FInvhedL3> getLast3InvoiceHed(String debcode) {
        ...
        String selectQuery = "select * from "+ValueHolder.TABLE_FINVHEDL3+" where DebCode ='" + debcode + "' ORDER by TxnDate DESC limit 3 ";
