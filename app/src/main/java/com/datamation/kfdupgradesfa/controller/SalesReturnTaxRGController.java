...
import com.datamation.kfdupgradesfa.model.TaxRG;
            ...
            for (FInvRDet invRDet : list) {
                ...
                if (invRDet.getFINVRDET_RETURN_TYPE().equals("MR") || invRDet.getFINVRDET_RETURN_TYPE().equals("UR")) {
                    ...
                    for (TaxDet taxDet : taxcodelist) {
                        ...
                        String s = "SELECT * FROM " + TABLE_INVRTAXRG + " WHERE " + ValueHolder.REFNO + "='" + invRDet.getFINVRDET_REFNO() + "' AND " + INVRTAXRG_TAXCODE + "='" + taxDet.getTAXCODE() + "'";
                        ...
                        values.put(INVRTAXRG_TAXCODE, taxDet.getTAXCODE());

                        if (cursor.getCount() <= 0)
                            count = (int) dB.insert(TABLE_INVRTAXRG, null, values);
