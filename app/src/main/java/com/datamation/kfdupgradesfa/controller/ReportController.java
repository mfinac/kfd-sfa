...
///////////////////////////////////////////get target vs actual data //////////////////////////////////////////////////
    public ArrayList<Target> getTargetVsActuals(String from, String to) {//developed by rashmi
        ...
        String selectQuery = "SELECT itd.txndate, itd.Rdtarget, ifnull((sum(a.Amt)),0)  as totAmt " +
                "FROM TblItenrDet itd " +
                "LEFT JOIN TblOrddet a " +
    ...
    public ArrayList<DayExpDet> getDaExpenseData(String from, String to) {
        ...
        String selectQuery = "SELECT d.ExpCode,r.ExpName, d.Amt from TblExpense r, TblDayExpDet d  where d.expcode = r.expcode  and d.txndate between '" + from + "' and "
                +"'" + to + "'";
...
//                +"'" + to + "' group by d.ItemCode";
        String selectQuery = "select d.itemcode,d.barcode,d.variantcode,h.debcode,d.qty,d.amt,d.txndate,i.itemname, count(distinct h.debcode)as reach   from \n" +
                "TblItem i,Tblinvdet d, Tblinvhed h  " +
        ...
        try {
            while (cursor.moveToNext()) {
                ...
                ordDet.setFORDDET_SEQNO(""+cursor.getInt(cursor.getColumnIndex("reach")));
